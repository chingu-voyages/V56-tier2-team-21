...
function App() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      ...
      <button
        onClick={() => setshowChatbot((prev) => !prev)}
        className="bg-blue-600 hover:bg-blue-700 text-white rounded-full p-3 shadow-lg transition duration-300"
      >
        {showChatbot ? (
          <X className="w-6 h-6" />
        ) : (
          <MessageCircle className="w-6 h-12" />
        )}
      </button>

      {/* Chatbot Popup */}
      {showChatbot && (
        <div className="mt-3 w-[350px] max-h-[90vh] bg-white rounded-xl shadow-2xl flex flex-col overflow-hidden animate-fade-in">
          {/* Header */}
